...
Scales determineScaling(Function fun)
{
    ...
    {
        if (trial > 0 && ((times[trial - 1] * sizes[trial]) > 10))
        {
            break;
        }
        setA = &setsA[trial];
    ...
    linear = checkLinear(times, sizes, 0, trial - 1) || checkLinear(times, sizes, 1, trial - 1);
    sublinear = checkSubLinear(times, sizes, 0, trial - 1) || checkSubLinear(times, sizes, 1, trial - 1);
